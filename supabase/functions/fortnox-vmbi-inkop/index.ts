...
async function getFortnoxApiDocs(endpoint?: string, method?: string) {
  try {
    ...
    console.log('📚 Could not fetch API docs:', (error as Error).message || 'Unknown error');
  }
  return null;
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🚀 NEW VERSION: fortnox-vmbi-inkop function started')
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid JSON in request body', details: (parseError as Error).message || 'Invalid JSON' }),
    ...
    console.log(`🚀 Starting VMBI project creation for inventory item: ${inventoryItemId}, syncing user: ${syncingUserId}`)
    ...
    // Check if vehicle vat_type is VMBI
    if (inventoryItem.vat_type !== 'VMBI') {
      console.log(`ℹ️ Vehicle vat_type is ${inventoryItem.vat_type}, not VMBI. Skipping project creation.`)
      return new Response(
        JSON.stringify({ message: 'Vehicle vat_type is not VMBI, project creation skipped' }),
    ...
    try {
      ...
      // 🧾 Step: Create supplier invoice with proper VMBI accounting
      console.log('📋 Creating supplier invoice with VMBI accounting...');
      try {
        ...
        // Get account numbers - use user configured or fallback to defaults for VMBI
        const vmbiAccountNumber = accountNumberMap['Lager - VMB-bilar - EU'] || '1413';
        ...
        console.log(`📋 Using VMBI account number: ${vmbiAccountNumber} (user configured: ${!!accountNumberMap['Lager - VMB-bilar - EU']})`);
        ...
        if (leverantorskulderAccountNumber === '2440') {
          ...
          const supplierInvoiceRows = [
            {
              Account: vmbiAccountNumber, // VMBI inventory account (debit) - full purchase price
